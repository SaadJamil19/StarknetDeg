...
const LOCKER_ROLE_PRIORITY = Object.freeze([
  'exchange',
  'forwarder',
  'router',
  'market_manager',
...
const KNOWN_LOCKER_OVERRIDES = Object.freeze([
  {
    contractAddress: normalizeAddress('0x00f6f4cf62e3c010e0ac2451cc7807b5eec19a40b0faacd00cca3914280fdf5a', 'fibrous.router'),
    displayName: 'Fibrous',
    protocol: 'fibrous',
    protocolKey: 'fibrous',
    role: 'router',
    sourceUrls: [
      'https://docs.fibrous.finance/integrate-best-trading/fibrous-sdk/route-and-calldata',
    ],
  },
]);
...
const KNOWN_LOCKER_INDEX = buildMap(KNOWN_LOCKER_OVERRIDES, 'contractAddress');
...
function getProtocolDisplayName(protocolKeyOrProtocol) {
  if (!protocolKeyOrProtocol) {
    return null;
  }

  const normalized = String(protocolKeyOrProtocol).trim().toLowerCase();
  const directCatalogMatch = DEX_CATALOG.find((entry) =>
    entry.key === normalized || String(entry.protocol ?? '').trim().toLowerCase() === normalized);
  if (directCatalogMatch) {
    return directCatalogMatch.displayName ?? directCatalogMatch.protocol ?? directCatalogMatch.key;
  }

  const lockerOverrideMatch = KNOWN_LOCKER_OVERRIDES.find((entry) =>
    entry.protocolKey === normalized || String(entry.protocol ?? '').trim().toLowerCase() === normalized);
  if (lockerOverrideMatch) {
    return lockerOverrideMatch.displayName ?? lockerOverrideMatch.protocol;
  }

  return null;
}

function getKnownLockerMatchByAddress(address) {
  if (!address) {
    return null;
  }

  const normalizedAddress = normalizeAddress(address, 'locker address lookup');
  const overrideMatch = (KNOWN_LOCKER_INDEX.get(normalizedAddress) ?? [])[0] ?? null;
  if (overrideMatch) {
    return overrideMatch;
  }

  const staticMatches = getStaticMatchesByAddress(normalizedAddress);
  if (!staticMatches || staticMatches.length === 0) {
    return null;
  }

  const sortedMatches = [...staticMatches].sort((left, right) => {
    const leftPriority = LOCKER_ROLE_PRIORITY.indexOf(left.role);
    const rightPriority = LOCKER_ROLE_PRIORITY.indexOf(right.role);
    const normalizedLeftPriority = leftPriority === -1 ? LOCKER_ROLE_PRIORITY.length : leftPriority;
    const normalizedRightPriority = rightPriority === -1 ? LOCKER_ROLE_PRIORITY.length : rightPriority;
    if (normalizedLeftPriority !== normalizedRightPriority) {
      return normalizedLeftPriority - normalizedRightPriority;
    }
    return String(left.protocolKey ?? '').localeCompare(String(right.protocolKey ?? ''));
  });
