...
const DEADLOCK_BACKOFF_MS = Object.freeze([1_000, 2_000, 4_000]);
...
async function withTransaction(work) {
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      if (!isDeadlockError(error) || attempt >= DEADLOCK_BACKOFF_MS.length) {
        throw error;
      }

      const delayMs = DEADLOCK_BACKOFF_MS[attempt];
      console.warn(
        `[db] deadlock detected (code=${String(error?.code || '') || 'unknown'}), retrying transaction in ${delayMs}ms (attempt ${attempt + 1}/${DEADLOCK_BACKOFF_MS.length})`,
      );
      await sleep(delayMs);
    }
  }
...
function isDeadlockError(error) {
  ...
  if (String(error.code ?? '').trim().toUpperCase() === '40P01') {
    return true;
  }
