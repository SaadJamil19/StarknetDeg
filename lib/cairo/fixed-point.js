...
function resolveUsdPriceFromGraph({
  ...
  const visited = new Map([[normalizedTarget, 0]]);
  let best = null;
  ...
  while (queue.length > 0) {
    ...
    for (const edge of neighbors) {
      ...
      const visitedHops = visited.get(edge.toTokenAddress);
      if (visitedHops !== undefined && visitedHops <= nextHops) {
        continue;
      }

      visited.set(edge.toTokenAddress, nextHops);
      queue.push({
